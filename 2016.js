...
map.on('load', () => {
    ...
    map.addSource('CSD-2016', { // Adding the source for the CSD layer
        type: 'geojson',
        data: 'https://raw.githubusercontent.com/ryansiu1/ggr472-webpage/main/data/CSD_2016.json'
    });

    map.addLayer({ // Adding the 2016 CSD layer
        'id': '2016-Income-CSD',
        'source': 'CSD-2016',
        ...
        'paint': {
            'fill-color': [
                ...
                70211, '#c7e9c0', // Colours assigned to values >= each step
                89028, '#74c476',
                99701, '#238b45',
                106708, '#00441b'
            ],
    ...
    map.addSource('CT-2016', { // Adding the source for the CT layer
        type: 'geojson',
        data: 'https://raw.githubusercontent.com/ryansiu1/ggr472-webpage/main/data/CT_2016.json'
    });

    map.addLayer({ // Adding the 2016 CT layer
        'id': '2016-Income',
        'source': 'CT-2016',
    ...
    map.on('mousemove', '2016-Income-CSD', (e) => {
        if (e.features.length > 0) { //determines if there is a feature under the mouse
            map.setFilter('2016-Income-CSD-Fill', ['==', ['get', 'CSDUID'], e.features[0].properties.CSDUID]); //applies the filter set above
        }
    });

    map.on('mouseleave', '2016-Income-CSD-Fill', () => { //removes the highlight when the mouse moves away
        map.setFilter("2016-Income-CSD-Fill", ['==', ['get', 'CSDUID'], '']);
    });

    // Add a hover effect for the CT when the mouse is over it
    map.on('mousemove', '2016-Income', (e) => {
        if (e.features.length > 0) { //determines if there is a feature under the mouse
            map.setFilter('2016-Income-Fill', ['==', ['get', 'CTUID'], e.features[0].properties.CTUID]); //applies the filter set above
        }
    });

    map.on('mouseleave', '2016-Income-Fill', () => { //removes the highlight when the mouse moves away
        map.setFilter("2016-Income-Fill", ['==', ['get', 'CTUID'], '']);
    });
    ...
    map.on('click', '2016-Income-CSD', (e) => {
        new mapboxgl.Popup()
            .setLngLat(e.lngLat)
